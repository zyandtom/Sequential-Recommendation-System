...
class MultiSessionsGraph(InMemoryDataset):
    ...
    def __init__(self, root, phrase, transform=None, pre_transform=None):
        assert phrase in ['train_data', 'test', 'test_seq_data']
        self.phrase = phrase
    ...
    def process(self):
        ...
        df = pd.read_csv(self.root + '/' + self.raw_file_names[0], delimiter=':', names=names)
        test = df['sequence'] \
            .map(lambda x: x.split(',')).apply(lambda x: list(map(int, x)))
        testy = []
        for sequence in test:
            testy.append(sequence[-1])
        testx = test.apply(lambda x: x[1:]).tolist()
        data = [testx]
        data.append(testy)
