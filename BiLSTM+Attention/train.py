...
def train_model(args, data_loaders, data_lengths, DEVICE):
    model = BILSTM(args, data_lengths['nuniq_items'], DEVICE)
...
def test_inference():
    ...
    model = BILSTM(args, dataset.nuniq_items, DEVICE)
...
if __name__ == '__main__':
    ...
    data = torch.utils.data.DataLoader(dataset, args.batch_size)
    data_loaders = {"train": data, "val": vl_dl}
    data_lengths = {"train": len(data), "val": len(vl_dl), "nuniq_items": dataset.nuniq_items}
    print(DEVICE)
